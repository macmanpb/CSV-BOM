	...
	def notify(self, args):
		...
		if lastPrefs:
			try:
				...
				_includeBoundingboxDims = getattr(lastPrefs, "incBoundDims", True)
				_ignoreUnderscorePrefixedComps = getattr(lastPrefs, "ignoreUnderscorePrefComp", True)
				_underscorePrefixStrip = getattr(lastPrefs, "underscorePrefixStrip", False)
				_ignoreCompsWithoutBodies = getattr(lastPrefs, "ignoreCompWoBodies", True)
				_ignoreLinkedComps = getattr(lastPrefs, "ignoreLinkedComp", True)
				_ignoreVisibleState = getattr(lastPrefs, "ignoreVisibleState", True)
